...
print('load model')
checkpoint = torch.load(checkpoint_folder + '/' + resume_model, map_location=None if use_gpu else 'cpu')
...
try:
    ...
    while True:
        ...
        if valid:
            ...
            img = ori_img.copy()
            pre_x1 = pre_y1 = pre_x2 = pre_y2 = 0  # 当图当中没有手的时候，这些就是0
            ...
            if if_exist_hand:
                ...
                crop_img_input1 = torch.tensor(crop_img_input).to('cuda')
            ...
            if if_save:
                save_name = 'background_' + str(int(time.time() * 1000))
                if pre_x2 == 0 or pre_y2 == 0:  # 当图当中没有手的时候就不保存
                    pass
                else:
