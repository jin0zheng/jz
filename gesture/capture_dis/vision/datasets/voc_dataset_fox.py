...
class VOCDataset:
    ...
    def __init__(self, root, transform=None, target_transform=None, is_test=False, keep_difficult=False):
        ...
        self.ids = VOCDataset._read_image_ids(image_sets_file)  # 列表，所有图片的名字
        self.keep_difficult = keep_difficult

        logging.info("No labels file, using default VOC classes.")
        self.class_names = ('BACKGROUND', 'hand')  # 这块需要改，只有在这里面的才能被挑出来
    ...
    def _read_image(self, image_id):
        ...
        # image = image[:, :, np.newaxis]    # 灰白的读出来是个二维的，所以要加上一维
        return image
