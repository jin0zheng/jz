...
def listener_():
    with Listener(on_move=None, on_click=on_mouse_event, on_scroll=None) as listener:
        listener.join()
    ...
    while True:
        ...
        if valid:
            ...
            if if_save:
                ...
                cv2.imwrite(path + '/' + "single_%d_" % n + str(int(time.time() * 1000)) + '.jpg', ori_img)
            else:
                cv2.waitKey(8)
        if if_save == " ":
            break
